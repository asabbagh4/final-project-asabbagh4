 ...
 *
 * HRES and VRES are not defined here; they come from
 * capturelib.h, which this header includes. Redefining
 * them here would cause redefinition errors in any
 * translation unit that also includes v4l2_interface.h.
 *
 ...
 * including HRES, VRES, MAX_HRES, MAX_VRES, and MAX_PIXEL_SIZE.
 * Including it here means any TU that includes only process_frame.h
 * also has access to those constants without an additional include. */
 ...
 * can compute a pixel-wise absolute difference without retaining the full
 * acquisition ring buffer. The ring is intentionally small: only the
 * immediately preceding frame is needed for a frame-to-frame comparison.
 *
 * Members:
 *   ring_size  - Fixed capacity of frames[]; always 2.
 *   tail_idx   - Slot index that will receive the next incoming frame.
 *                Wraps modulo ring_size after each store.
 *   count      - Number of valid frames currently stored (0, 1, or 2).
 *                Guards comparisons until a baseline frame exists.
 ...
 * or consumed by store_frame.c. Sized for the largest possible raw frame:
 * MAX_HRES * MAX_VRES * MAX_PIXEL_SIZE bytes. */
 ...
 *                              unsigned char *b)
 *    returns    : void
 *    y          : luminance sample, range 0-255
 *    u          : Cb chrominance sample, range 0-255 (128 = neutral)
 *    v          : Cr chrominance sample, range 0-255 (128 = neutral)
 ...
 *                 arithmetic. Coefficients are scaled by 256 and right-shifted
 *                 by 8 bits to approximate the BT.601 floating-point matrix
 *                 without an FPU.
 ...
 *                 for each YUYV macropixel pair. Not used in the
 *                 COLOR_CONVERT_GRAY path.
 **************************************************/
 ...
 *                 frame in motion_buffer using a per-pixel absolute difference
 *                 sum. Normalizes the total against the theoretical maximum
 *                 (frame_size * 255) to produce a percentage. Declares motion
 *                 if the percentage exceeds 0.4%. In all cases, stores
 *                 current_frame into motion_buffer for the next call.
 * Notes         : The 0.4% threshold was determined empirically; lower values
 *                 increase sensitivity but also false-positive rate.
 ...
 *                 any stored baseline frames. Call when reinitializing the
 *                 pipeline, after a known abrupt scene change, or whenever
 *                 the comparison baseline should be treated as invalid.
 * Notes         : Not thread-safe; call only when the processing service
 *                 thread is not concurrently executing process_motion_detection().
 **************************************************/
 ...
 *                 advances the head index and decrements ring_buffer.count
 *                 to reflect the slots consumed. Updates fnow for
 ...
 * so the complete function inventory of this module is visible in one place
 * and future unit-test harnesses can reference it without modifying the
 * source file. Do not call it from outside this module.
 ***************************************************/

/**************************************************
 * Function name : static int process_image(const void *p, int size)
 *    returns    : running process_framecnt after this call
 *    p          : pointer to the raw frame in the acquisition ring buffer
 *    size       : byte length of the raw frame (HRES * VRES * PIXEL_SIZE)
 * Created by    : course 4 team
 * Date created  : 2024
 * Description   : Dispatches the raw frame to the correct color-space
 *                 conversion path based on the pixel format in the global
 *                 fmt struct negotiated during V4L2 initialization:
 *
 *                   V4L2_PIX_FMT_GREY:
 *                     Frame is already single-channel grayscale;
 *                     no conversion performed.
 *
 *                   V4L2_PIX_FMT_YUYV + COLOR_CONVERT_RGB:
 *                     Unpacks each 4-byte macropixel [Y0 U Y1 V] to two
 *                     RGB triplets (6 bytes) in scratchpad_buffer via
 *                     yuv2rgb().
 *
 *                   V4L2_PIX_FMT_YUYV + COLOR_CONVERT_GRAY:
 *                     Extracts bytes 0 and 2 of each macropixel (Y0, Y1)
 *                     into scratchpad_buffer, yielding size/2 luma bytes,
 *                     then calls process_motion_detection().
 *
 *                   V4L2_PIX_FMT_RGB24:
 *                     Frame is already RGB24; no conversion needed.
 *
 * Notes         : Increments process_framecnt before dispatching so the
 *                 count reflects all frames seen regardless of outcome.
